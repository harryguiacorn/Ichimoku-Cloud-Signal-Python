...
def fillcol(label):
    logger.debug("fillcol label: %s", label)
    if label >= 1:
...
# include averages
fig.add_traces(
    go.Scatter(x=df1.index, y=df1.ma1, line=dict(color="blue", width=1))
)

fig.add_traces(
    go.Scatter(x=df1.index, y=df1.ma2, line=dict(color="red", width=1))
)

# include main time-series
fig.add_traces(
    go.Scatter(x=df1.index, y=df1.y, line=dict(color="black", width=2))
)
